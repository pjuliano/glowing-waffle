...
BEGIN
    ...
    LOOP
        ...
        b_ :=   ar.objid;
        c_ :=   ar.objversion;
        d_ :=   'PAY_TERM_BASE_DATE'||chr(31)||ar.invoice_date||chr(30)||
                'DUE_DATE'||chr(31)||ar.due_date||chr(30)||
                'CURR_RATE'||chr(31)||'1'||chr(30)||
                'LANGUAGE_CODE'||chr(31)||'English'||chr(30)||
                'VOUCHER_TEXT'||chr(31)||ar.customer_name||chr(30)||
                'USER_GROUP'||chr(31)||'AC'||chr(30)||
                'UPDATE_CUST_PAY_PLAN'||chr(31)||'FALSE'||chr(30);
        e_ :=   'DO';
        IFSAPP.MAN_CUST_INVOICE_API.MODIFY__( a_ , b_ , c_ , d_ , e_ );

        EXECUTE IMMEDIATE 'UPDATE kd_pt_arbalances SET stage = 2 WHERE invoice = ''' || ar.invoice_id || '''';
    END LOOP;
END;
