DECLARE
   ...
   b_ VARCHAR2(32000) := '20-70001P'; --p1
   c_ VARCHAR2(32000) := '*'; --p2
   d_ VARCHAR2(32000) := 'BURLINGTON'; --p3
   e_ VARCHAR2(32000) := 'WO-005996'; --p4
   f_ VARCHAR2(32000) := '*'; --p5
   g_ VARCHAR2(32000) := '1'; --p6
   h_ VARCHAR2(32000) := '*'; --p7
   i_ DATE := to_date('2021-01-22-00.00.00','YYYY-MM-DD-HH24.MI.SS','NLS_CALENDAR=GREGORIAN'); --p8
   j_ FLOAT := 4; --p9
   k_ FLOAT := NULL; --p10
...
BEGIN
    ...
    LOOP
        ...
        g_  := '1'; --p6
        h_  := '*'; --p7
        i_  := to_date(to_char(to_Date(parts.exp_date,'MM/DD/YYYY'),'YYYY-MM-DD:HH24.MI.SS'),'YYYY-MM-DD-HH24.MI.SS','NLS_CALENDAR=GREGORIAN'); --p8
        j_  := parts.qty; --p9
        k_  := NULL; --p10
        l_  := '9998'; --p11
        m_  := '0001'; --p12
