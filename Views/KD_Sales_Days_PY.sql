Create OR Replace View KD_Sales_Days_PY As
With Days_Of_Year As (
    Select
        To_Date('01/01/' || (Extract(Year From Sysdate)-1),'MM/DD/YYYY') + Rownum - 1 As Day,
        Calendar_Api.Get_Week_Day(To_Date('01/01/' || (Extract(Year From Sysdate)-1),'MM/DD/YYYY') + Rownum - 1) As Weekday
    From
        All_Objects
    Where
