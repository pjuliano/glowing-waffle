...
Select
    ...
    E.Units_Sold_12m,
    E.Avg_Monthly_Sales,
    Round((B.RMFG_Split_Total + Nvl(C.On_Hand_Proc,0) + Nvl(C.Open_PO_Proc,0) + Nvl(C.Open_SO_Proc,0) + Nvl(CC.On_Hand_Proc,0) + Nvl(CC.Open_PO_Proc,0) + Nvl(CC.Open_SO_Proc,0) + Nvl(D.On_Hand_Fg,0) + Nvl(D.Open_So_Fg,0))/Nullif(E.Avg_Monthly_Sales,0),2) As Months_On_Hand
From
